# v7 版本（v6 + 禁止自我引用与重复保存）

> 本文件由 buildExtractionPrompt() 自动导出，保证与代码一致。
> 相对 v6 的改动：落盘规约新增第 3 条【严禁自我引用与重复保存】。
>
> 起因（铁证）：fork 抽取时实测发现模型的真实行为序列为
>   1) memhub_save 写入正确的卡（kb-d3c1ffda）
>   2) memhub_get 读回自己刚写的卡
>   3) memhub_recent 列出最近卡片
>   4) 误判为"上次的检查点"，再次 memhub_save 并 supersedes 掉正确的那张
> 结果：最终落库的是被污染的"续接/checkpoint"版本，主干内容丢失。
>
> 修复：明确禁止抽取后读库、禁止把库中已有卡当作前情、禁止 supersedes/覆盖。
>
> 注：本版依赖提示词约束（用户选择），未在机制层限制工具集。
