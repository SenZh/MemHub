# v9 版本（related_files 改为可选 + 明确无文件也可沉淀）

> 本文件由 buildExtractionPrompt() 自动导出，保证与代码一致。
>
> 相对 v8 的改动（来自 v8 实测暴露的两个新问题）：
> 1. **related_files 由"必填"改为"可选"**：v8 实测 S3 列出 34 个文件（把会话读过的全塞了），
>    现改为"只列承载本卡核心知识的 1-5 个文件，不要求穷尽"，测试文件/设计文档/只读浏览过的一般不必列。
> 2. **明确"没有文件也要正常沉淀"**：v8 实测 S4（OMS 权限 5W2H 梳理）抽 0 张，而 v7 能抽出 1 张。
>    推测是"必填文件清单"让模型误以为无文件的会话不符合要求。现明确：本项是可选信息，不是沉淀前提。
> 3. 落盘段的 related_files 描述同步改为"可选"。
