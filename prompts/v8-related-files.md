...
>
> 相对 v7 的改动（来自 3 个 subagent 盲评的意见）：
> 1. **新增第三节【关键涉及文件】（必填项）**：v7 多张卡写"暂无关联文件"，
>    但正文里明明提到了具体文件路径。现要求正文末尾必须单列文件清单，
>    并同步通过 related_files 参数落库。
> 2. **强化【严禁工程过程痕迹】**：明确列出 commit hash、提交推送动作
>    （"已提交/已推送/已交付"）、测试用例数量、文件行号、分支名、执行流水。
>    起因：v7 的 S3/S4 仍残留"已交付并推送""758 例全通过"。
> 3. **新增【可执行细节要保留】**：对冲过度精简——明确"具体文件路径、请在 X 环境验证、
>    精确时间窗口（UTC 01:00–02:00）、SQL/配置片段、滚动重启清单"不算水分，写具体比笼统好。
>    起因：评审指出 v4 的"UTC 01:00–02:00"这类可执行粒度比 v7 更实用。
> 4. 节号顺延（三->四 标题、四->五 tags、五->六 落盘）。
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
三、关键涉及文件（必填项，不得省略）：
- 在正文之后，必须单独列出本次会话实际涉及的核心文件/路径清单（每行一个，用 `-` 开头，带 ` 包裹）。
- 来源包括：会话中读取/修改的代码文件、配置、SQL、文档；正文中提到过的每条路径都要收进来。
- 路径必须与会话中出现的完全一致，不要凭猜测构造路径。
- 如果本次会话确实没有涉及任何文件（如纯概念讨论、纯线上排查），写"无"。
- 格式示例：
  ```
  ## 关键涉及文件
...
1. 若具备沉淀价值，必须调用 memhub_save 工具进行保存，且【只调用一次】，参数规范：
   ...
   - content: 按【目标】【背景】【方案】【结论】【经验】五段式撰写的高密度 Markdown 正文（方案可省，经验无则不写），结尾附第三节要求的"## 关键涉及文件"清单
   - related_files: 与正文"关键涉及文件"相同的路径数组（如 ["a/b.xml","c/d.properties"]；若无则传空数组）
