...
> **版本定位**：v0.1.2 记忆提炼、认知读门禁与做梦自省交付版（Consolidation & Dreaming Release）  
...
| **6. 后台常驻定时提炼守护 (P0)** | `memhub daemon` 常驻自循环，通过 OpenCode HTTP 驱动宿主 LLM 抽取 | 已完整打透（`src/daemon.js`、`src/host/opencode-client.js`）：动态端口与鉴权 + 7天窗口/120分静默 + 权威排除所有 Subagent 子会话 + 严禁假卡 | **100%** |
...
| **9. 离线做梦与记忆熔炼 (P1 - 顶层热点)** | 极简 Cron 触发闲时做梦（Dreaming）：碎片三维聚类、宿主 LLM 反思 Prompt、防重状态机、L4 升华层落盘与碎片封存 | Phase 1 & 2 端到端已完整打通（Prompt 模板、执行流水线、supersedes 溯源链、consolidated 封存流转），单测 100% 覆盖 | **100%** |
...
### 1. 离线做梦与记忆熔炼引擎 (`src/dream/`)
- **三维聚类亲和度算法** (`src/dream/clustering.js`)：
  - 基于 $Score = 0.35 \times Jaccard(Tags) + 0.30 \times Overlap(Files) + 0.35 \times CosineSim(Vectors)$，通过 BFS/DFS 连通子图将 2~5 张高凝聚力碎片切分为独立做梦主题簇；
- **架构师级做梦反思提示词** (`src/dream/prompt.js`)：
  - 输入多碎片详细要素，指导宿主 LLM 交叉比对共性技术根因、扫描设计冲突（Contradiction Radar），提炼通用的 L4 规范；
- **做梦调度与状态机闭环** (`src/dream/pipeline.js`)：
  - 落盘 L4 卡片（`is_synthesized = 1`，`supersedes = "id1,id2..."` 挂载完整溯源链）；
  - 将原底层碎片原子更新为 `status = 'consolidated'` 与 `consolidated_into = l4_id`，永久退出后续做梦候选池，杜绝套娃；
  - 写入 `knowledge_dream_history` 审计表，30 天内相同卡片组合绝对幂等拦截；
- **夜间极简 Cron 与 CLI** (`src/daemon.js`, `src/cli.js`)：
  - 支持 `memhub dream`（支持 `--dry-run`, `--affinity`, `--project`），常驻守护在凌晨自动触发做梦自省。

### 差距 2：动态知识地图自动注入 `memhub map` (P2)
...
│ • 项目维度研发态势与资产大盘矩阵 (memhub stats 项目对比)           │
│ • 项目命名空间防腐治理 (统一 oms 权威收敛与 Prompt 强约束)         │
│ • 本地 384 维向量 + FTS5 Trigram + RRF 倒数排名混合检索           │
│ • 后台常驻提炼守护 memhub daemon (OpenCode HTTP 动态端口与鉴权驱动)│
│ • 离线做梦与记忆熔炼 (Dreaming Phase 1 & 2：三维聚类+L4熔炼+封存) │
└─────────────────────────────────┬────────────────────────────────┘
                                  │
                                  ▼
┌──────────────────────────────────────────────────────────────────┐
