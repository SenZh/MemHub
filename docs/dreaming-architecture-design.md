...
### 4. WHEN（何时触发？—— 定时调度与生命周期）
* **极简定时驱动（Cron 调度，最简单可靠）**：
  * 默认配置标准 Cron 表达式：`"0 3 * * *"`（每天凌晨 03:00 自动做梦反思）；
  * 不搞复杂的进程树负载探针，符合工程 KISS 原则；开发者睡醒后即可看到昨夜做梦成果。
* **显式指令触发**：开发者或测试随时在终端执行 `memhub dream --dry-run` 立即进行聚类与反思演练；
* **幂等防空转**：若当前知识库无可熔炼的新碎片簇，秒级结束，0 算力浪费。
